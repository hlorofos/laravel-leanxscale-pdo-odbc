...
namespace LaravelPdoOdbc\Database\Schema\Grammars;
...
class LeanSchemaGrammar extends Grammar
{
    /**
    ...
    {
        return "select * from sysmeta.tables where tableSchem = ? and tableName = ? and tableType = 'TABLE'";
    }
     ...
     * @return string
     */
    public function compileGetAllTables(string $database)
    {
        // TODO check if we'll need this * columns.
        return "select tablename,* from sysmeta.tables where tableSchem = '$database' and tableType = 'TABLE'";
    }
    ...
    protected function getColumns(Blueprint $blueprint)
    {
        ...
        foreach ($blueprint->getAddedColumns() as $column) {
            ...
            // For Lean we have to move default column settings AFTER modifiers.
            if (preg_match('/ default /i', $sql)) {
                $sql = preg_replace(
                    '/ default /',
                    $this->addModifiers('', $blueprint, $column) . ' default ',
                    $sql
                );
            } else {
...
//                }
    ...
    protected function compileKey(Blueprint $blueprint, Fluent $command, $type)
    {
        ...
        return sprintf('alter table %s add %s (%s)',
            $this->wrapTable($blueprint),
            $type,
            // We do not support unique index name
//            $this->wrap($command->index),
            // We do not support various indexing algorithms.
//            $command->algorithm ? ' using '.$command->algorithm : '',
     ...
     * TODO check the actual allowed length of varchar.
     * @param \Illuminate\Support\Fluent $column
     * @return string
     */
    ...
    {
        // Lean doesn't support unsigned numbers.
        return '';
        if ($column->unsigned) {
            return ' unsigned';
        }
...
//            return ' auto_increment primary key';
            return ' AS IDENTITY start with 1 increment by 1';
     ...
     * Lean doesn't support backticks, so we're using " here.
     * TODO: this method comes from Grammar, not from MySqlGrammar. Get rid of it when base class would be Grammar.
     * @param string $value
     * @return string
     */
